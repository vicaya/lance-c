...
use lance::dataset::fragment::FragmentCreateBuilder;
...
pub unsafe extern "C" fn lance_fragment_create(
    ...
    ffi_try!(unsafe { fragment_create_inner(spool_uri, schema, stream) }, neg)
}
...
unsafe fn fragment_create_inner(
    ...
    let spool_uri = unsafe { helpers::parse_c_string(spool_uri)? }.ok_or_else(|| {
        Error::InvalidInput {
